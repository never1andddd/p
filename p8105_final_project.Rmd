...
---
# Motivations
Nowadays, customer reviews in social media significantly influence the chances of success of any business. Restaurant customers look for a complete and satisfactory experience regarding food quality, service and they often seek reviews of others when they are choosing where to eat. Yelp offers this information to its users. When users look for a place to eat, they can search for a list of restaurants. Users also get the overall rating(stars) of a restaurant that other customers gave to the restaurant and some reviews about the restaurant.
  
Reviews about a restaurant might differ from person to person. They can talk about the quality of food, the service and the environment; they might give a positive review or complain about some specific aspect of their experience. Therefore, reviews are full of information and usually give more information than a simple numeric rating. Fortunately, Yelp receives thousands of reviews each day from all over the world and extracting specific pieces of information from such a big dataset is challenging.

#Related work
Things we discussed in class:  
Data Wrangling II - Tidy Text http://jeffgoldsmith.com/DSI/tidy_text.html  
...
What words are frequently mentioned in the negative and positive reviews? How does the category of a restaurant influence the rating(stars)?  
How did these questions evolve over the course of the project? 
What new questions did you consider in the course of your analysis? Can we recommend a restaurant for a customer?

# Data
The data used in this project is part of the Yelp Dataset Challenge. The dataset consists of a set of JSON files that include business information, reviews, tips, user information and check-ins. Variables in Business are name, location, opening hours, category, average star rating, the number of reviews about the business and a series of attributes like noise level or reservations policy. Review objects list a star rating, the review text, the review date, and the number of votes that the review has received.In this project, we have focused on these two type of objects.  

Source: https://drive.google.com/drive/folders/190oLdoSyZVnydl9Jxth8G7aKzu3bDgee?usp=sharing  
The original review json: https://drive.google.com/file/d/1wbIRfISw2ZW0zy4bZGtnRyhUEOImi5_H/view?usp=sharing  
The r code to clean the review json: https://drive.google.com/file/d/1EXrW_F_GmEq-tQlmtnYkMDwtt-IOA3tq/view?usp=sharing  
Scraping method: We use the stream_in function in package "Jsonlite" to read in the data. Then We filtered the business by category to keep only those businesses in the restaurant category and in state of "AZ" (10219). We only extract first 5000 reviews from the original review json.
...
# assign one category with the highest frequency to each restaurant
final.dat = final.dat %>%
...
# a bar chart showing the number of restaurant in each category
```{r}
final.dat%>% 
