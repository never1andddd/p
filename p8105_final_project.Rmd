...
ggplot(aes(fill=as.factor(stars), color=as.factor(stars), ymax=ymax, ymin=ymin, xmax=5, xmin=4), data=data_reviews_agg) +
 ...
 theme(panel.grid=element_blank(), axis.text=element_blank(), axis.ticks=element_blank(), panel.background=element_blank(), axis.title.x = element_blank(), axis.title.y=element_blank(),legend.title = element_blank(),  legend.key.height=unit(0.25, "cm"), legend.margin=unit(-0.5,"cm"),panel.border= element_blank()) +
  ...
  scale_color_manual(values=rank_colors, labels = c("1 Star", "2 Stars", "3 Stars", "4 Stars", "5 Stars")) +
     annotate("text", x = 0, y = 0, label = "ring plot", col="#1a1a1a", family="Source Sans Pro Light", size=11)
```

We did a proportion ring chart of star rating in reviews. We had expected that the 5-star and 1-star reviews would have the largest proportion. Because people tend to express and share their feelings and experience when they had the most positive or negative experiences.  However, from the ring plot, we can find out the review numbers decrease as the star rating decreases. And 60% of the reviews have 4 or higher star ratings. It seems people are more likely to put in the effort to write a review when having a great dining experience.
